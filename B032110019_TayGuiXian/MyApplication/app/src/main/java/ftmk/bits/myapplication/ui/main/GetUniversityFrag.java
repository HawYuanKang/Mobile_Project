...
import ftmk.bits.myapplication.databinding.FragmentGetUniversityBinding;
...
//    }
    ...
    private void fnSearchUni(View view){
        String strURL = "http://universities.hipolabs.com/search?name=" + binding.edtFindUniversityByName.getText().toString()
                + "&country=" + binding.edtFindUniversity.getText().toString();
        RequestQueue requestQueue = Volley.newRequestQueue(getContext());
        StringRequest stringRequest = new StringRequest(Request.Method.GET, strURL, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                binding.results.setText(response+" End");
                binding.results.setMovementMethod(new ScrollingMovementMethod());
            }
        }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
                Toast.makeText(getContext(), "Unable to connect to University list!" + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
